...
Dans un second temps, nous souhaiterons pouvoir réaliser une vidéo animée représentant les trajets cyclistes qui se sont déroulés sur une journée que l'on aura préalablement déterminé.
...
Un code couleur sera utilisé pour indiquer le niveau de trafic :
...
# Choix des packages
- Pandas : Nous utiliserons le package pandas pour lire et manipuler les données du fichier CSV contenant les informations des trajets de vélos.
...
- Scipy : Nous utiliserons ce package pour les calculs avancés de probabilités et pour modéliser les comportements de trafic afin de prévoir des trajectoires cyclistes optimisées.
...
Cette table de données répertorie les trajets effectués via la branche Vélomagg de la Tam dans Montpellier sur l'année 2022. 
